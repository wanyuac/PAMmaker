...
# Creation: 5 Sep 2016, latest edition: 1 Nov 2020
...
if [[ "${stages[@]}" =~ "3" ]]; then
    ...
    if [ ! -f "${out_path}/cluster_table.txt" ]; then
        ...
        python3 ${code_path}/utility/tabulate_cdhit.py ${out_path}/clusters.fna.clstr > ${out_path}/cluster_table.txt  # run the next command as long as this command successed
    else

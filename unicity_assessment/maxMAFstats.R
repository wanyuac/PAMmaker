...
replaceAlleleIDs <- function(pam, score_tab) {
    ...
    # make a strain-allele table from the PAM
...
maxMAFmatrix <- function(apam, score_table, sep = "\t") {
    ...
    if (cls == "character") {
        scores <- read.table(file = score_table, header = TRUE, sep = sep, stringsAsFactors = FALSE)  # first column: strain names
    } else if (cls == "data.frame") {
        scores <- score_table
    } else if (cls == "matrix") {
    ...
    scores$Allele <- replaceAlleleIDs(pam, scores)  # Some allele names become NA afterwards.
    ...
    for (s in strains) {
        pa <- pam[s, ]  # a named vector of integers
